    ...
    for (uint bi = 0u; bi < blocks; ++bi) {
        ...
        if (col < pc.n) {
            ...
            for (uint hsel = 0u; hsel < 2u; ++hsel) {
                ...
                uint k0 = hsel * 128u + 0u * 32u + l;
                uint k1 = hsel * 128u + 1u * 32u + l;
                uint k2 = hsel * 128u + 2u * 32u + l;
                uint k3 = hsel * 128u + 3u * 32u + l;

                sum += shared_a[k0] * (d * float(sc0) * float(qv0));
                sum += shared_a[k1] * (d * float(sc1) * float(qv1));
                sum += shared_a[k2] * (d * float(sc2) * float(qv2));
                sum += shared_a[k3] * (d * float(sc3) * float(qv3));
            }
        }
        barrier();
    }
