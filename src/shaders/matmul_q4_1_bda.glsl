...
float q41Weight(uint row, uint kidx) {
    uint blocks = (pc.k + 31u) / 32u;
    uint b = kidx / 32u;
    uint i = kidx % 32u;
    uint base = (row * blocks + b) * 20u;
    float d = f16ToF32(getU16(pc.b, base + 0u));
    float m = f16ToF32(getU16(pc.b, base + 2u));
    uint qb = getByte(pc.b, base + 4u + i / 2u);
    if ((i & 1u) == 0u) {
        float qv = float(qb & 0x0Fu);
        return (qv * d) + m;
    } else {
        float qv = float(qb >> 4);
        return (qv * d) + m;
...
void main() {
    ...
    for (uint t = 0u; t < tiles; ++t) {
        ...
        b_tile[ly][lx] = (col < pc.n && ky < pc.k) ? q41Weight(col, ky) : 0.0;
        barrier();
